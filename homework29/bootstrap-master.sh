...
then
	...
	#sudo mysql -uroot -p$DBPASSWD -e "GRANT ALL PRIVILEGES ON $DBNAME.* TO 'betscraper'@'localhost' IDENTIFIED BY '$DBPASSWD'"
...
then
	...
	sshpass -p "$REPLICA_SSH_PASS" ssh -o StrictHostKeyChecking=no $REPLICA_SSH_USER@$REPLICA_IP "mysql -uroot -p$DBPASSWD $DBNAME < /vagrant/config/$DBNAME.dmp"
